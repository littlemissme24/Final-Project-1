...
void combat() {
	combatHub();
	int playerAttack;
	//sets player and monster damage
	int playerDamage = 9 * character.level / 2;
	int monsterAttack = 7 * monsterLevel / 2;
	// list combat steps
	if (character.totalHealth >= 1 && monsterHp >= 1) {
		...
		if (playerAttack == 1) {
			...
			if (monsterHp >= 1) {
				...
				// If the monster has gotten you below 0 health you are dead
				...
				//Helps character level up after killing a monster and receiving XP
			...
			//random number with a 50% change of blocking a attack
			int i = rand() % 100 + 1;
			if (i >= 50) {
...
void characterMoving()
{
	...
	if (decision == 1) {
		...
		Sleep(1300);
		mainHub();
	}// allows the character to heal when taking a nap.
	else if (decision == 2) {
		...
		if (temp >= 50) {
			...
			cout << "You found a helpless cat as you were walking. \n You stopped to give it some food and water then continue on your way. " << endl;
			Sleep(1300);
			mainHub();
		}
	} // Saves it from breaking the same if user inputs a invalid decision.
	else {
		cout << " That was a invalid decision" << endl;
		Sleep(700);
		characterMoving();
